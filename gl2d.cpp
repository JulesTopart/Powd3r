...
void gl2D::paintGL(){
    ...
    if(activeSlice < _lines.size() && activeSlice != 0){
        glBegin(GL_POLYGON);
        glColor3f(0.5f, 0.5f, 0.5f);
        for(size_t i(0); i < _lines[activeSlice].size(); i++){
            glVertex3f(_lines[activeSlice][i].A().x, _lines[activeSlice][i].A().y, 0);
            glVertex3f(_lines[activeSlice][i].B().x, _lines[activeSlice][i].B().y, 0);
        }
        glEnd();
    }
}
